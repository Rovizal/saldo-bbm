...
class Mobil extends MX_Controller
{
	...
	public function get_perjalanan_datatable()
	{
		$id = $this->input->get('data');

		$idMobil = $this->base64UrlSafeDecode($id);
		$query = $this->datatable_model->get_activity_datatable_mobil($idMobil);
	...
	function get_edit_request()
	{
		...
		$id_mobil 			= $this->base64UrlSafeDecode($id);
		$mobil 				= $this->global_model->get_mobil_by_id($id_mobil)->row();
		$getMerekId			= $this->global_model->get_merek_by_name($mobil->merk)->row('id');
		$getTipeByMerekId	= $this->global_model->get_tipe_by_merek_id($getMerekId)->result();

		echo json_encode([
			'statusCode' 	=> 200,
			'data' 			=> $mobil,
			'merk_id' 		=> $getMerekId,
			'tipe_mobil' 	=> $getTipeByMerekId,
		]);
		return;
	}
	...
	public function update_mobil()
	{
		...
		if (!empty($_FILES['gambar_mobil']['name'])) {
			...
			if (!$this->upload->do_upload('gambar_mobil')) {
				...
				$full_path = FCPATH . 'uploads/' . $oldFile;
				if (!empty($oldFile) && file_exists($full_path)) {
					unlink($full_path);
				}
			}
		}
		...
		$updateResult = $this->global_model->update_mobil($mobil_id, $data);
		...
		if ($this->db->trans_status() === FALSE || !$updateResult) {
			echo json_encode([
				'statusCode' => 500,
				'message' => 'Gagal update data mobil.'
			]);
			return;
		}
